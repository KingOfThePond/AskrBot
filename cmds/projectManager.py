...
class project_manager(object):
    ...
    async def do_command(self, message):
        ...
        if text == 'add' and ('Suppirts' in [role.name for role in message.author.roles] or 'Souppirts' in [role.name for role in message.author.roles]):
            ...
            await client.send_message(message.channel, text_out)
            ...
            if ('Suppirts' in [role.name for role in message.author.roles] or 'Souppirts' in [role.name for role in message.author.roles]):
                if project_name.lower() in self.project_names:
                    await client.send_message(message.channel,'Do you really want to delete this project? Y/N')
                    msg = 'placeholder'
                    while not (msg.lower() == 'y' or msg.lower() == 'n'):
                        msg = await client.wait_for_message(author=message.author)
                        msg = msg.content
                        if not (msg.lower() == 'y' or msg.lower() == 'n'):
                            await client.send_message(message.channel,'Please answer Y or N.')
                    if msg.lower() == 'y':
                        ...
                        await client.send_message(message.channel,'Project deleted.')
                    elif msg.lower() == 'n':
                        await client.send_message(message.channel,'Project was not deleted.')

                else:
                    await client.send_message(message.channel,'Project not found.')
                ...
                await client.send_message(message.channel,'You must be a mod to do this.')

        elif text == 'list':
            all_projects = 'Our current projects:\n'
            for project in self.project_list:
                all_projects += ' - ' + self.project_list[project].name + '\n'
            await client.send_message(message.channel, all_projects)
            ...
            if project_name in self.project_names:
                if message.author.id not in self.project_list[project_name].participants:
                    ...
                    await client.send_message(message.channel, 'You\'re already in this project!')
            else:
                await client.send_message(message.channel,'Project not found.')
            ...
            if project_name in self.project_names:
                if message.author.id not in self.project_list[project_name].participants:
                    ...
                    self.write_projects()
                    await client.send_message(message.channel, 'You have left this project.')
            else:
                await client.send_message(message.channel,'Project not found.')
